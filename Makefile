...
run: all
	@if [ -f $(ISO_FILE) ]; then \
		qemu-system-i386 -drive file=kfs.iso,format=raw,index=0,media=disk -m 8 -serial file:output.log -serial stdio; \
	else \
...
debug: all
	@if [ -f $(ISO_FILE) ]; then \
		qemu-system-i386 -drive file=kfs.iso,format=raw,index=0,media=disk -s -S -m 8 -serial file:output.log -serial stdio -display curses; \
	else \
		echo "No $(ISO_FILE) found, please run 'make' first."; \
	fi
