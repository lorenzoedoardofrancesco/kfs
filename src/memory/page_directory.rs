...
pub fn init_pages() {
    ...
    let mut tables = [PageTable::new(); ENTRY_COUNT];

	// Map all the virtual addresses to the corresponding physical address
	for (i, table) in tables.iter_mut().enumerate() {
		...
		//let phys = tables[i].entries.as_ptr() as u32;
		//directory.add_entry(i, phys, PageDirectoryFlags::PRESENT | PageDirectoryFlags::WRITABLE);
	}

	// Map the kernel to the virtual address 0xC0000000
	let phys = tables.as_ptr() as u32;
	directory.add_entry(0x300, phys, PageDirectoryFlags::PRESENT | PageDirectoryFlags::WRITABLE);

	// Map the page directory to the virtual address 0xFFFFF000
	let phys = &directory as *const _ as u32;
	directory.add_entry(0x3ff, phys, PageDirectoryFlags::PRESENT | PageDirectoryFlags::WRITABLE);

	// Load the page directory
	unsafe {
        // Load the page directory base address into CR3
        asm!("mov cr3, {}", in(reg) &directory);

        // Read CR0, set the paging (PG) bit, and write it back
        let mut cr0: u32;
